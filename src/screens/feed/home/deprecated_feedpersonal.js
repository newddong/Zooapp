...
import {getPostListById, getMorePost} from '../feedapi';
...
export default FeedPersonal = ({navigation, route}) => {
	...
	const likedPostList = React.useRef([]);
	...
	React.useEffect(() => {
		...
		switch (context.current.datamode) {
			case FIRST_DATA:
				scroll && scroll.current.scrollToOffset({offset: POSTHEIGHT * context.current.index, animated: false});
	...
	const onMomentumScrollEnd = e => {
		...
		if (e.nativeEvent.contentOffset.y < POSTHEIGHT && !context.current.isDataLoading) {
			context.current.isDataLoading = true;
